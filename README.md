...
`spec` blocks, `raises`, native `java { }` blocks, property-based `ensures`, a Jakarta Mail
binding for the `mail` effect (the interface is the binding point), and persistence for
`Map` fields and lists of identified entities.
